...
export function useAxios () {
  ...
  axios.interceptors.response.use(
    ...
    (error) => {
      if (error.response.status === 401 && window.location.pathname !== LOGIN.path) {
  ...
  function prepareHeaders (rest: AxiosRequestConfig): AxiosRequestConfig {
    ...
    return accessToken.length ?
      { headers: { Authorization: `Bearer ${accessToken}` }, signal: controller?.signal, ...rest }
  ...
  const api: AxiosHttpType = {
    ...
    delete: (path: string, data?: object, params: AxiosRequestConfig = {}) => {
      const config = prepareHeaders(params)
